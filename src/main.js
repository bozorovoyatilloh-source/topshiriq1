...
function updateLastSync() {
  if (el.lastSync) {
    el.lastSync.textContent = `Last sync: ${timeNow()}`;
  }
}
...
async function refreshAllData() {
  if (!state.readContracts) {
    handleError("Avval contracts.json yuklangan bo'lishi kerak", "Umumiy yangilash");
    return;
  }

  try {
    await Promise.all([refreshGreeting(), refreshVoting()]);
    showToast("Barcha view ma'lumotlar yangilandi", "success");
  } catch (err) {
    handleError(err, "Umumiy yangilash");
  }
}

async function copyWalletAddress() {
  if (!state.account) {
    handleError("Wallet hali ulanmagan", "Wallet nusxalash");
    return;
  }

  try {
    await navigator.clipboard.writeText(state.account);
    showToast("Wallet manzili nusxalandi", "success");
  } catch (err) {
    handleError(err, "Wallet nusxalash");
  }
}
...
function registerEvents() {
  ...
  el.refreshAll?.addEventListener("click", refreshAllData);
  el.copyWallet?.addEventListener("click", copyWalletAddress);
